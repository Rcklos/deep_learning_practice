...
# 表情分类器
model = load_model('./tensor/xception_.65-1.01.hdf5', compile=False)
# 表情
...
def emotion_classify(face):
    img = np.asarray(face).reshape(input_size[::-1])
    eval = predict(img)
    ...
    for face in faces:
        if face.any():
            ...
            if face_img.any():
                ...
                resize_img = cv.resize(face_img, input_size[:2])  
                # cv.imshow('resize', resize_img)
                emotion = emotion_classify(resize_img)
                log.info(emotion)
                cv.putText(frame, emotion, (face[0], face[1]), cv.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 1)
